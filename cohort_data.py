...
def unique_houses(filename):
    ...
    """
    house_report = open(filename)
    houses = set()
    for line in house_report:
        line = line.rstrip()
        words = line.split("|")
        if words[2] != "":
            houses.add(words[2])
...
def hogwarts_by_house(filename):
    ...
    all_students = [sorted(list1) for list1 in all_students]
...
def all_students_tuple_list(filename):
    ...
    for line in cohort:
        ...
        if words[2] == "":
            continue
        name = words[0] + " " + words[1]
        student = (name, words[2], words[3], words[4])
...
def find_cohort_by_student_name(student_list):
    ...
    for toople in student_list:
        if name == toople[0]:
            return ("%s was in %s cohort") % (name, toople[3])
